...
# 33 class for only mask to train?
def dice_loss_mutil(score, target ,ignore=None):
    total_dice = 0
    clss = torch.unique(target).tolist()
    for cls in clss:
        if cls == 0:
            continue
        total_dice += dice_loss(score[:, cls, :, :, :], target==cls, ignore)
    total_dice /= len(clss)
    return total_dice
...
class DiceLoss(nn.Module):
    ...
    #     return loss
    def _dice_loss(self, score, target, ignore):
        return dice_loss(score, target, ignore)
