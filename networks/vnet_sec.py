...
class Upsampling(nn.Module):
    def __init__(self, n_filters_in, n_filters_out, stride=2, normalization="none"):
        ...
        ops.append(nn.Conv3d(n_filters_in, n_filters_out, kernel_size=7, padding=3))
        if normalization == "batchnorm":
            ops.append(nn.BatchNorm3d(n_filters_out))
        elif normalization == "groupnorm":
            ops.append(nn.GroupNorm(num_groups=16, num_channels=n_filters_out))
...
class VNet_sec(nn.Module):
    def __init__(
        ...
        self.block_nine = ConvBlock(
            1, n_filters + 1, n_filters, normalization=normalization
        )
    ...
    def decoder(self, features, x_bin=None, no_drop=False):
        ...
        x8_up = x8_up + x1
        # print(x8_up.shape)
        # print(x8_up.shape, x_bin.shape)
        x8_up = torch.cat([x8_up, x_bin], dim=1)
        # print(x8_up.shape)
    ...
    def forward(self, input, x_bin,ret_feats = False, drop=False):
        x_bin = x_bin.detach()
        # print(input.shape, x_bin.shape)
        if ret_feats:
            ...
            out = self.decoder(features, x_bin)
...
if __name__ == "__main__":
    net = VNet_sec(n_channels=1, n_classes=33, n_filters=16, normalization="batchnorm")
    a = torch.randn(1, 1, 112,112,80)
    m = torch.randn(1, 1, 112,112,80)
