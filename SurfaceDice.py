...
def compute_iou_score(mask_gt, mask_pred):
    """Compute Intersection over Union (IoU).

    compute the Intersection over Union between the ground truth mask `mask_gt`
    and the predicted mask `mask_pred`.
    ...
    Returns:
      the IoU score as float. If both masks are empty, the result is NaN
    """
    volume_sum = mask_gt.sum() + mask_pred.sum()
    if volume_sum == 0:
        return np.NaN
