...
from networks.vnet_multitask import VNetMultiTask, Corr3D
...
parser.add_argument("--exp", type=str, default="train_Bin", help="model_name")
parser.add_argument(
    "--max_iterations", type=int, default=9000, help="maximum epoch number to train"
)
parser.add_argument("--batch_size", type=int, default=2, help="batch_size per gpu")
parser.add_argument(
    "--base_lr", type=float, default=0.001, help="maximum epoch number to train"
)
parser.add_argument(
...
# n_filters
parser.add_argument('--n_filters', type=int, default=16, help='n_filters')
# 

args = parser.parse_args()

num_classes = 2
num_classes_multi = 33
n_filters = args.n_filters

dice_loss = DiceLoss(n_classes=num_classes)
dice_loss_multi = DiceLoss(n_classes=num_classes_multi)
...
if __name__ == "__main__":
    ...
    net = VNetMultiTask(
        n_channels=1, n_classes_1=num_classes, n_classes_2=num_classes_multi, normalization="batchnorm", n_filters=n_filters, 
    )
    net = net.cuda()
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, (
            ...
            mask_x_bin = (mask_x > 0).long()
            img_u_w = img_u_w.cuda()
            img_u_s1, img_u_s2 = img_u_s1.cuda(), img_u_s2.cuda()
            
            bef_time = time.time()

            pred_x_pred_u_w, pred_x_pred_u_w_multiclass, pred_x_b_pred_u_w_b = net(
                torch.cat((img_x, img_u_w)), ret_feats=True, drop=False, out_multiclass=True
            )
            # print(f'{pred_x_pred_u_w.shape = }, {pred_x_pred_u_w_multiclass.shape = }, {pred_x_b_pred_u_w_b.shape = }')
            pred_x, pred_u_w = pred_x_pred_u_w.chunk(2)
            pred_x_multiclass, pred_u_w_multiclass = pred_x_pred_u_w_multiclass.chunk(2)
            # bottleneck features
            ...
            loss_x = (
                F.cross_entropy(pred_x, mask_x_bin)
                + dice_loss(pred_x, mask_x_bin)
            ) / 2.0
            
            loss_x_multiclass = (
                F.cross_entropy(pred_x_multiclass, mask_x)
                + dice_loss_multi(pred_x_multiclass, mask_x)
            ) / 2.0
            ...
            if args.s1_to_s2:
                loss = (loss_x + loss_u_s1 * 0.1667 + loss_u_s2 * 0.1667 + loss_s1_s2 * 0.1667) / 1.5 + loss_x_multiclass 
            else:
                loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25) / 1.5 + loss_x_multiclass
            ...
            writer.add_scalar("loss/loss_x_multiclass", loss_x_multiclass, iter_num)
            writer.add_scalar("loss/loss", loss, iter_num)
            writer.add_scalar("loss/loss_u_s1", loss_u_s1, iter_num)
            writer.add_scalar("loss/loss_u_s2", loss_u_s2, iter_num)
            if args.s1_to_s2:
                writer.add_scalar("loss/loss_s1_s2", loss_s1_s2, iter_num)
        ...
        if (epoch_num + 1) % (4) == 0:
            ...
            with torch.no_grad():
                ...
                dice = test(net_fake, image_list, roi_size=patch_size)
