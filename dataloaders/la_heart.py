...
class Tooth_WCS(Dataset):
    """LA Dataset"""

    def __init__(
        self, base_dir=None, mode="train", num=None, transform=None, id_path=None, full_size=False
    ):
        self._base_dir = base_dir
        self.transform = transform
        self.sample_list = []
    ...
    def __getitem__(self, idx):
        ...
        image = h5f["ROI_image"][:]
        label = h5f["ROI_label"][:] if 'label' in h5f.keys() else np.zeros_like(image)
        ...
        if self.mode == "train_l":
            return (image).float(), (label).long()
        if self.mode == "test":
            return (image).float(), (label).long(), image_name
...
class Tooth_sec(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = h5f["image"][:]
        label = h5f["label"][:] if 'label' in h5f.keys() else np.zeros_like(image)
        
        # img, mask => resize (112, 112, 80)
        ...
        # print(sample["image"].shape, sample["label"].shape, sample1['label'].shape)
        sample_auged = self.transform(sample)
        image, label = sample_auged["image"], sample_auged["label"]
...
class Resize3D(object):
    """
    Resize a 3D image (and optionally a label or additional channel) to a new size.
    Args:
    output_size (tuple of int): Desired output size (depth, height, width)
    """

    def __init__(self, output_size):
        assert isinstance(output_size, (tuple, list)) and len(output_size) == 3, "output_size should be a tuple or list of three integers"
        self.output_size = output_size
    ...
    def __call__(self, sample):
        ...
        zoom_factors = (depth / image.shape[0], height / image.shape[1], width / image.shape[2])

        # Resize image
        resized_image = zoom(image, zoom_factors, order=1)  # Using bilinear interpolation (order=1) for images
        resized_label = zoom(label, zoom_factors, order=0)  # Using nearest interpolation (order=0) for labels
        resized_sample = {'image': resized_image, 'label': resized_label}
